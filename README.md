...
- `/devpanl:add-storybook` — opt-in: scaffold `stories/`, the sync workflow, and the UI section in `CLAUDE.md` so this project publishes to https://ui.devpanl.dev/&lt;slug&gt;/. Idempotent. Pair with `scripts/wire-storybook-secrets.sh` to provision the GitHub Actions secrets in one shot.
